...
@section('content')
<form id="form-perempuan" class="flex-grow items-center justify-center ">
  ...
  <div class="items-center flex justify-between mt-8 gap-8 ">
    <a href="/perempuan" id="kembali_laki_laki" type="button"
...
<script>
  $(document).ready(function() {
    ...
    $.ajax({
      url: `/api/perempuans/${id}`, // Ganti dengan URL endpoint server Anda
    ...
    $('#form-perempuan').on('submit', function(event) {
      ...
      $.ajax({
        url: `/api/perempuans/${id}?_method=PUT`, // Ganti dengan URL endpoint server Anda
        ...
        success: function(response) {
          ...
          Swal.fire({
            ...
            window.location.href = '/perempuan';
          });
        },
        error: function(xhr, status, error) {
          // Jika terjadi error, tampilkan SweetAlert
